...
//p622 연결 리스트를 이용한 조세푸스 문제의 구현	// !@#9_1_1
...
자바의 LinkedHashMap 을 c++ 로 구현하려면 어찌하면 되나: // !@#9_1_2
	먼저 LinkedHashMap 이란 "입력된 순서"를 기록한 놈임 : 입력된 순서대로 순회를 빠르게 할수 있다. 
	n 개 입력된 상태에서 순회 수행 -> O(n) 걸림
	- 특징
...
"How to Replace the value in particular key in LinkedHashMap" : https://stackoverflow.com/questions/27705908/how-to-replace-the-value-in-particular-key-in-linkedhashmap
"Changing the value for an existing key" won't change the iteration order.
// key에 대한 값을 갱신하더라도 삽입 순서대로 순회한다. : 디폴트
_LHmap["aaa"] = 3;
_LHmap["bbb"] = 5;
_LHmap["ccc"] = 7;
_LHmap["aaa"] = 9;
iteraotr 순서 진행은 "aaa" -> "bbb" -> "ccc" 키 순서대로 동작한다.

// 물론 너는 LinkedHashMap iteration 을 insertion-order or access-order 둘중 하나로 선택도 가능하다.
// insertion-order 가 디폴트일 뿐이다.!!!!!!!!




// 이정도만해도 됨	
입력된 순서를 리스트 형태로 저장, 삭제는 신경쓰지 않음
bbb-aaa-ddd-eee-ccc	// aaa가 삭제된 상태라 치자.
순회하는 경우 중간에 key값인 aaa 가 존재하지 않으면 해당 리스트 지운다.
bbb-ddd-eee-ccc

// 삭제시 리스트를 바로 갱신하는 짜는 방법
https://codereview.stackexchange.com/questions/197814/a-c-implementation-of-linkedhashmap // 잘못된 구현이나 참고할 것이 많음 c++ 문법부터 !!!!!!!!
...
// 못돌아가는 코드임
int main() {
    unordered_map<string, pair<int, list<string>::reverse_iterator>> uMap;
    list<string> hashKeyList;

    hashKeyList.push_back("ccc");
    uMap["ccc"] = make_pair(2, hashKeyList.rbegin());


    hashKeyList.push_back("aaa");
    uMap["aaa"] = make_pair(0, hashKeyList.rbegin());;


    hashKeyList.push_back("bbb");
    uMap["bbb"] = make_pair(1, hashKeyList.rbegin());;


    hashKeyList.push_back("zzz");
    uMap["zzz"] = make_pair(26, hashKeyList.rbegin());;


    hashKeyList.push_back("eee");
    uMap["eee"] = make_pair(4, hashKeyList.rbegin());;


    //pair<int, list<string>::reverse_iterator> val = uMap["aaa"];
    //list<string>::reverse_iterator it = val.second;     // it 는 eee 를 가르키고 있다. ㅅㅂ
    //it = hashKeyList.erase(it); // 컴파일 오류: reverse_iterator 로 지우는게 안되넹
	// reverse_iterator 는 요상하게 동작하네.. eee만 가르키고 + erase 는 안되고 썅
	
    list<string>::iterator it = hashKeyList.begin();
    it = hashKeyList.erase(it);  
...
// 잘동작하는 수정 버전임
int main() {
    unordered_map<string, pair<int, list<string>::iterator>> uMap;
    list<string> hashKeyList;

    hashKeyList.push_back("ccc");
    uMap["ccc"] = make_pair(2, --(hashKeyList.end()));


    hashKeyList.push_back("aaa");
    uMap["aaa"] = make_pair(0, --(hashKeyList.end()));;


    hashKeyList.push_back("bbb");
    uMap["bbb"] = make_pair(1, --(hashKeyList.end()));;


    hashKeyList.push_back("zzz");
    uMap["zzz"] = make_pair(26, --(hashKeyList.end()));;


    hashKeyList.push_back("eee");
    uMap["eee"] = make_pair(4, --(hashKeyList.end()));;
	
	// 실사용에선 키 "aaa" 를 지울려고 한다면 : 먼저 "aaa"가 uMap 에 존재하는지 체크부터 해야한다. 
	// if (uMap.fid("aaa") != uMap.end())
    pair<int, list<string>::iterator> val = uMap["aaa"];
    hashKeyList.erase(val.second);
    uMap.erase("aaa");

    list<string>::iterator it = hashKeyList.begin();
    for (; it != hashKeyList.end(); ++it) {
        string key = *it;
        cout << key << ": " << uMap[key].first << '\n';
    }

    {
        // 살짝 불안해요 end 앞에 원소가 없으면 안되요 (위 코드는 문제 없지만)
        list<string> tmp;
        list<string>::iterator jt = tmp.end();
        //--jt;   // 죽는다능.
    }
...
https://stackoverflow.com/questions/2678175/iterator-to-last-element-in-stdlist/2678214
#include <list>
using std::list;
...
https://gomguard.tistory.com/115 // LRU 설명, kakao lru 캐싱 문제 링크도 있다
// 릿코드에도 있다.
https://leetcode.com/problems/lru-cache/discuss/45976/C%2B%2B11-code-74ms-Hash-table-%2B-List
class LRUCache {
private:
	typedef list<int> LI;	typedef pair<int, LI::iterator> PII;	typedef unordered_map<int, PII> HIPII;
	HIPII cache;
	LI used;
	int _cp;
	void touch(HIPII::iterator it) {
		int key = it->first;
		used.erase(it->second.second);	// it->second.second 는 list iterator 이다.
		used.push_front(key);
		it->second.second = used.begin();
	}
public:
	LRUCache(int cp) : _cp(cp) {}
	int get(int key) {
	...
	void set(int key, int value) {
		auto it = cache.find(key);
		if (it != cache.end()) 
			touch(it);	// 원소가 이미 있는 경우 리스트에서 순서만 갱신한다.
		else {
			if (cache.size() == _cp) {
				// 꽉찬 경우 맨 마지막 원소를 지운다.: 해시랑 리스트 둘다
...
public class LRUCache extends LinkedHashMap<Integer, Integer> {
	...
	public int get(int key) {
		return super.getOrDefault(key, -1);	// 값이 존재하지 않을 시, -1반환
	}
	public void put(int key, int value) {
		super.put(key, value);
	}
	
	// 아래 조건을 만족하는 경우, 가장 오래된 요소를 삭제한다. 재정의하지 않으면 기본 값은 false 이다.
	@Override
...
// LinkedHashSet 의 경우 더미일 뿐이다... 순서 설정을 할수가 없다.
    public LinkedHashSet() {
        super(initialCapacity: 16, loadFactor: .75f, dummy: true);
    }

Set<Integer> myTest = new LinkedHashSet<>();
myTest.add(3);
myTest.add(5);
myTest.add(7);
myTest.add(1);
...
// [2]: 7
// [3]: 1
// 중복된것이 들어와도 order 가 변경하지 않는다.
Iterator <Integer> it = myTest.iterator();
while (it.hasNext()) {
	Integer t = it.next();	// t 는 3, 5, 7, 1 을 가진다.
	t = 10;
}
...
#include <list>
#include <string>
//using namespace std;		// 헤더에선 절대 쓰지마
...
class LinkedMap {
private:
	//std::map<Key, std::pair<Val, std::list<Key>::iterator>> m_map;	// iterator 는 '_Ty2' 매개 변수에 대한 올바른 템플릿 형식 인수가 아닙니다. 
	//std::list<Key> m_order;									// iterator 가 색상 자체가 흰색임. 즉 컴파일러가 ::iterator 를 타입으로 인식을 못함 


	std::map<Key, std::pair<Val, typename std::list<Key>::iterator>> m_map;	// 명시적인 타입 지정 방법을 취하면 된다. : 참고 : http://youngrok.net/wp/?p=125
	std::list<Key> m_order;


	//std::list<Key> m_order;
	//std::map<Key, decltype(m_order.begin())> m_map;		// decltype 을 쓰려면 m_order 에 대한 정의가 위에 있어야 한다. !!!!
	bool m_bInsertOrder;
public:
	LinkedMap(bool insertOrder = true) : m_bInsertOrder(insertOrder) {	}
	void put(Key key, Val val) {
		...
		if (m_map.count(key) != 0) {
			...
			m_map[key].first = val;
			m_map[key].second = --m_order.end();
		}
	}

	void erase(Key key) {
		std::pair<Val, std::list<Key>::iterator> element = m_map[key];
		m_order.erase(element.second);
		m_map.erase(key);
	}
	...
	std::list<Key> getOrderKey() {
		return m_order;
	}

	// 이건 어느경우 사용해야 할까..
	// 만약 key 의 value 값이 다른 값으로 변경된다고 하더라도 list 의 순서는 최초 insert 순으로 보장되어야 한다.
	Val& operator [] (Key key) {
		if (count(key) == 0)
			put(key, Val());
		return m_map[key].first;
	}
	// const 객체에 대한 [] 호출을 위해 추가할 필요가 있을까... const 로 하니 컴파일 에러난다. ㅠㅠ
};
...
class LinkedSet {
private:
	std::map<Ele, typename std::list<Ele>::iterator> m_map;	// 명시적인 타입 지정 방법을 취하면 된다. : 참고 : http://youngrok.net/wp/?p=125
	std::list<Ele> m_order;

public:
	LinkedSet() {	}
	void insert(Ele ele) {
		// 객체가 기존에 존재하는 경우 아무것도 안한다.
		if (m_map.count(ele) != 0) {

		}
		else {
			m_order.push_back(ele);
			m_map[ele] = --m_order.end();
		}
	}
	...
	bool erase(Ele ele) {
		if (m_map.count(ele) == 0) 
			return false;
		typename std::list<Ele>::iterator it = m_map[ele];
		m_order.erase(it);
		m_map.erase(ele);
		return true;
	}
	...
	int count(Ele ele) {
		return m_map.count(ele);
	}
	std::list<Ele> getOrderKey() {
		return m_order;
	}
...
// 상인의 마음으로 사고 팔고를 반복해서 최대 이익을 거두라. 하루에 사거나 팔거나 한가지만 가능하며 파는건 이전에 산 경우만 가능
class Solution {
public:
    int maxProfit(vector<int>& prices) {
        ...
        for (int i = 0; i < prices.size(); ++i) {
            ...
            // 스택의 사이즈가 1인 경우 => 뒤에 큰숫자가 들어오면 push 한다.
            //                         => 뒤에 작은 숫자가 들어오면 현재놈을 대체한다.
            
            // 스택의 사이즈가 2인 경우   => 뒤에 큰숫자가 들어오면 그놈으로 대체한다.
            //                          => 뒤에 작은 숫자가 들어오면 두놈을 pop 하고 현재놈을 push
...
vector<int> kmpSearch(const string& H, const string& N) {
	...
	while (begin <= h - n) {	//while (begin + n <= h)	// while (begin + matched < h) 는 어떤가 해보자
...
kmp 를 바늘 찾기가 아닌 부분일치갯수에 중점을 두고 보아라. 그래야 응용력이 생긴다. !!!!!!!!

//가능한 짧은 펜린드롬 만들기(PALINDROMIZE, 하) // [회문][palindrome]
...
bool isChild(int parent, int child) {
	...
	for (int i = 0; i < n; ++i)
		if (i != parent && i != child && 
			encloses(parent, i) && encloses(i, child)) //!!!! 안보고 외워 짤때 잘 실패한 부분
...
//음... 다시 생각보니 키에 대한 value 가 없다면 키중복 처리를 안해줘도 됨.. 즉 아래 함수를 그대로 사용해도 무방할 것이야...
...
// BST(Binary Search Tree) 를 잎의 depth 차가 최대 1인 BST 로 변경하라. 
"BST 특성: 노드의 왼쪽 자식은 부모보다 작다, 노드의 오른쪽 자식은 부모보다 크다."
...
public:
    ...
    TreeNode* sortedArrayToBST(int start, int end) {
        if (start > end) return NULL;	// 좋은 표현이다. !!!!!!!!
        int mid = (start + end) / 2;
...
//변화하는 중간값 RUNNINGMEDIAN, 난이도 하		// !@#9_2_1
...
// 각각의 원소들이 입력되고 입력된 그 순간마다 최대 k개를 선택해서 합을 구해서 저장하라.	// !@#9_2_2
vi solve(vi& arr, int k) {
	priority_queue<int, vi, greater<int>> pq;		// 최상단이 가장 작음
	vi ret;
	long long sum = 0;
	for (int i = 0; i < arr.size(); ++i) {
		pq.push_back(arr[i]);
...
// 통과함. 응용하기 좋다. 
// (speed들의 k개 이하 합) * min(k 개 이하의 efficiency들) = 최대값을 구하는 문제 
class Solution {
public:
    int maxPerformance(int n, vector<int>& speed, vector<int>& efficiency, int k) {
		...
		sort(arr.begin(), arr.end(), greater<pair<int, int>>());	// {4, 3}, {4, 2}, {1, 2}
		...
		for (int i = 0; i < arr.size(); ++i) {
			...
			ret = max(ret, sum * arr[i].first);
		}
		ret %= 1000000007;
		return ret;
    }
};
// 윗 문제를 처음엔 삼분서치로 접근했는데 원천적으로 문제가 있었기에 생략한다. (게다가 삼분서치는 단조 구간이 존재하면 적용하기 위험하다.)
...
struct RangeResult {
	int size;
	bool allok;
	ll rightSum, leftSum, mSum;	// mSum 은 중간을 가로 지르는 최대값
...
RangeResult merge(const RangeResult& a, const RangeResult& b) {
	...
	if (a.allok && b.allok)	// a전체가 연결되어 있으면 && b전체가 연결되어 있으면
		ret.allok = true;
	else
		ret.allok = false;

	if (ret.allok)
		ret.mSum = ret.rightSum;	// leftSum이나 rightSum 둘 중 아무거나 됨
	else {
		ll cand = max(a.mSum, b.mSum);	// 왼쪽의 중간, 오른쪽의 중간
		cand = max(cand, ret.leftSum);	// 왼쪽 부분 합
		cand = max(cand, ret.rightSum);	// 오른쪽 부분합
		cand = max(cand, a.rightSum + b.leftSum);	// 중간을 거치는 합
		ret.mSum = cand;
	}
	return ret;
}
...
// Petr mitrichev 의 구간 갱신, 구간 질의 fenwick 트리 이용
// https://petr-mitrichev.blogspot.com/2013/05/fenwick-tree-range-updates.html
class Fenwick {
public:
    // 분석해보니 많은 갱신 수행시 값의 오버플로우 위험이 존재하넹... 음음.. long long 으로 변경
    // 같은 상태를 표현하는 dataMul , dataAdd 값이 여러가지가 발생하기에 엄청나게 많은 갱신이 필요한 곳엔 제한된 사용을 해야겠다.
...
int getSecond(string& str) {
    ...
    int ret = s + m * 60 + h * 3600;
    return ret;
}
string getStringTime(int num) {
    int h = num / 3600;
    int m = (num % 3600) / 60;
    int s = num % 60;
    string ret;
...
string solution(string play_time, string adv_time, vector<string> logs) {
    int play_t = getSecond(play_time);
    int adv_t = getSecond(adv_time);
    Fenwick ft(60 * 60 * 105);
    for (int i = 0; i < logs.size(); ++i) {
        string st = logs[i].substr(0, 8);
        string ed = logs[i].substr(9, 8);
        int st_t = getSecond(st);
        int ed_t = getSecond(ed);
        ft.update(st_t, ed_t - 1, 1);
    }
    long long sol = 0;
    int start = 0;
    for (int i = 0; i < play_t; ++i) {
        ...
        if (i != 0)
            psumS = ft.query(i - 1);
        psumE = ft.query(i + adv_t - 1);
        long long cand = psumE - psumS;
        if (sol < cand)
        {
            sol = cand;
            start = i;
    ...
    string ret = getStringTime(start);
    return ret;
}
int main() {
	ios::sync_with_stdio(false); cin.tie(0);
    ...
    string sol = solution(pt, et, logs);
	return 0;
}
...
#define rep(i,a) for (int i = 0; i < (a); ++i)
using namespace std;
vector<string> getCombi(const string & str) {
	vector<string> ret;
	for (int subset = 1; subset <= (1 << str.size()) - 1; ++subset) {
		string add;
...
vector<string> solution(vector<string> orders, vector<int> course) {
	map<string, int> my;
	for (int man = 0; man < orders.size(); ++man) {
		sort(orders[man].begin(), orders[man].end());
		vector<string> combi = getCombi(orders[man]);
		for (int i = 0; i < combi.size(); ++i)
			my[combi[i]]++;
	}
	map<string, int>::iterator it = my.begin();
	map<int, vector<string>> selection;
	map<int, int> mxCnts;		// 최대 몇번 주문했는가 기록용
	for (; it != my.end(); ++it) {
		string str = it->first;	// 선택
		int len = str.size();	// 선택길이
		int cnt = it->second;	// 선택 횟수
		if (len == 1) continue;
		if (cnt == 1) continue;
		if (mxCnts[len] < cnt) {
			selection[len].clear();
			selection[len].push_back(str);
			mxCnts[len] = cnt;
		}
		else if (mxCnts[len] == cnt) {
			selection[len].push_back(str);
		}
	}
	vector<string> ret;
	for (int i = 0; i < course.size(); ++i) {
		for (int ss = 0; ss < selection[course[i]].size(); ++ss) {
			ret.push_back(selection[course[i]][ss]);
		}
	}
...
int getAnd(bitset<50050>& arr, bitset<50050>& brr, bitset<50050>& crr, bitset<50050>& drr, bitset<50050>& vrr, int n) {
	bitset<50050> tmp;
	tmp = (arr & brr) & (crr & drr) & vrr;
	int ret = min(n, (int)tmp.count());
	return ret;
}
int getLanguage(string& str) {
	if (str.find("cpp") != string::npos)
		return 0;
	else if (str.find("java") != string::npos)
		return 1;
	else if (str.find("python") != string::npos)
		return 2;
	return -1;
}
int getJob(string& str) {
	if (str.find("backend") != string::npos)
		return 0;
	else if (str.find("frontend") != string::npos)
		return 1;
	return -1;
}
int getExp(string& str) {
	if (str.find("junior") != string::npos)
		return 0;
	else if (str.find("senior") != string::npos)
		return 1;
	return -1;
}
int getFood(string& str) {
	if (str.find("chicken") != string::npos)
		return 0;
	else if (str.find("pizza") != string::npos)
		return 1;
	return - 1;
}
int getVal(string& str) {
	...
	for (int i = 0; i < str.size(); ++i)
		if ('0' <= str[i] && str[i] <= '9')
			num.push_back(str[i]);
	int ret = atoi(num.c_str());
	return ret;
}
vector<int> solution(vector<string> info, vector<string> query) {
	// bitset<50050> arr, brr, crr, drr;
	map<int, bitset<50050>> am, bm, cm, dm;
	vector<pair<int, int>> valToIdxArr;
	for (int i = 0; i < info.size(); ++i) {
		int idxA = getLanguage(info[i]);
		am[idxA].set(i, true);
		int idxB = getJob(info[i]);
		bm[idxB].set(i, true);
		int idxC = getExp(info[i]);
		cm[idxC].set(i, true);
		int idxD = getFood(info[i]);
		dm[idxD].set(i, true);

		int val = getVal(info[i]);
		valToIdxArr.push_back(make_pair(val, i));
	}
	sort(valToIdxArr.begin(), valToIdxArr.end(), greater<pair<int,int>>());
	map<int, bitset<50050>> vm;
	bitset<50050> prv;
	for (int i = 0; i < valToIdxArr.size(); ++i) {
		int val = valToIdxArr[i].first;
		int idx = valToIdxArr[i].second;
		bitset<50050> here(prv);
		here.set(idx, true);
		vm[val] = here;
		prv = here;
	}
	...
	for (int i = 0; i < query.size(); ++i) {
		int idxA = getLanguage(query[i]);
		int idxB = getJob(query[i]);
		int idxC = getExp(query[i]);
		int idxD = getFood(query[i]);
		int idxV = getVal(query[i]);
		if (idxV != -1) {
			map<int, bitset<50050>>::iterator it = vm.lower_bound(idxV);
			if (it != vm.end())
				idxV = it->first;
		}
		int add = getAnd(am[idxA], bm[idxB], cm[idxC], dm[idxD], vm[idxV], info.size());
		sol.push_back(add);
	}
	return sol;
}
...
int solution(int n, int s, int a, int b, vector<vector<int>> fares) {
	s--; a--; b--; // 두사람이 s 지점에서 택시를 탄다. 동승할수도 있다. 그리고 a, b 목적지로 따로 내릴수 있다.
	for (int i = 0; i < n; ++i)
		for (int j = 0; j < n; ++j)
			adj[i][j] = INF;
	for (int i = 0; i < fares.size(); ++i) {
		int here = fares[i][0]; here--;
		int there = fares[i][1]; there--;
		int cost = fares[i][2];
		adj[here][there] = cost;
		adj[there][here] = cost;
	}
	...
	long long sol = INF;
	rep(i, n) {
		long long cand = adj[s][i] + adj[i][a] + adj[i][b];
		if (sol > cand)
			sol = cand;
	}
	return sol;
...
3. petr 소스는 0base 로 짜여져 있어서 이해하기 힘드니.. 이걸 참고 https://plzrun.tistory.com/entry/Fenwick-Tree-Range-Update-Range-Sum-%ED%8E%9C%EC%9C%85%ED%8A%B8%EB%A6%AC-%EA%B5%AC%EA%B0%84%EC%97%85%EB%8D%B0%EC%9D%B4%ED%8A%B8-%EA%B5%AC%EA%B0%84%ED%95%A9-OlgN%EC%97%90-%EA%B5%AC%ED%95%98%EA%B8%B0-feat-Segment-Tree-Lazy-Propagation
...
struct dsjSet {
	vi prnt, rank, size, mxNode;
	dsjSet(int n) : prnt(n), rank(n, 0), size(n, 1), mxNode(n) {
		rep(i, n) prnt[i] = i;
		rep(i, n) mxNode[i] = i;
	}
	int find(int u) {
	...
	int merge(int u, int v) {
		u = find(u); v = find(v);
		int mxU = mxNode[u], mxV = mxNode[v];
		if (u == v) return u;	// size[u]를 리턴해야 되지 않냐??
		if (rank[u] > rank[v]) swap(u, v);
		if (rank[u] == rank[v]) rank[v]++;
		prnt[u] = v;
		size[v] += size[u];
		mxNode[v] = max(mxU, mxV);
		return size[v];
	}
};
int main() {
...
// 카카오 2019 겨울 인턴십 호텔방 배정
// 방번호 선택 및 해당 방이 이미 선택된 방이면 그 오른쪽 방을 선택
// 값의 범위가 1000000000000 이라 disjoint set 을 그대로는 못쓴다.
// 대신 disjoint set 의 코드에서 경로 압축 방식을 이용할수 있다능
//int find(map<ll, ll>& my, ll here) {
// ㅅㅂ 효율성 맨 마지막 3개에서 틀렸다고 나왔는데 끝까지 못찾음: 눈으로 int 없다고 확신한게 문제였다.
// int 찾기를 했어야 했다... 눈으로 말고 ctrl+f 로 !!!!!!!!
long long find(map<ll, ll>& my, ll here) {
	if (my[here] == here)
		return here;
	return my[here] = find(my, my[here]);
}  
vector<long long> solution(long long k, vector<long long> room_number) {
	vector<long long> ret;
	map<ll, ll> my;	// 맨 오른쪽 기록
	for (int i = 0; i < room_number.size(); ++i) {
		...
		if (my.count(num) == 0) {
			...
			ll right = find(my, num);	// 부모를 찾고
			
			num = right + 1;			// 부모의 오른쪽이 다음 채울 부분이 된다.
			my[num] = num;				//
			ret.push_back(num);
		}
		if (my.count(num - 1) != 0)
